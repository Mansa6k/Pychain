...
#Check if a PyChain object exists in Streamlit session state.
def get_state():
    if "state" not in st.session_state:
        st.session_state.state = PyChainWrapper(PyChain([Block("Genesis", 0)]))
...
class PyChainWrapper:
    def __init__(self, pychain):
        self.pychain = pychain

@st.cache_data
def setup():
    print("Initializing Chain")
    return PyChainWrapper(PyChain([Block("Genesis", 0)]))

st.markdown("# PyChain")
st.markdown("## Store a Transaction Record in the PyChain")

pychain = get_state().pychain
...
if st.button("Add Block"):
    ...
    print(pychain.chain)  # Add this line to print the state of the PyChain
    st.balloons()
